...
#if 0
static int compute_vector_score(struct drone *drone, struct vec2d vector) {
	int fish_scores[MAX_ENTITIES] = {};

	vector.x /= COLLISION_POINTS_PER_VECTOR;
	vector.y /= COLLISION_POINTS_PER_VECTOR;

	for (int i = 0; i < COLLISION_POINTS_PER_VECTOR; i++) {
		struct vec2d drone_pos = { drone->x + (vector.x * i), drone->y + (vector.y * i) };

		for (int id = TOTAL_DRONE_COUNT; id < state.entity_count; id++) {
			struct fish *fish = &state.entities[id].fish;
			if (fish->type == -1) {
				struct vec2d monster_pos = { fish->x, fish->y };
				if (vec2d_distance(drone_pos, monster_pos) <= MONSTER_COLLISION_DISTANCE) {
					return INT_MIN;
				}
				continue;
			}

			if (is_scanned(drone, id)) { continue; }

			struct vec2d fish_pos;

			if (fish->visible) {
				fish_pos.x = fish->x + ((fish->vx / COLLISION_POINTS_PER_VECTOR) * i);
				fish_pos.y = fish->y + ((fish->vy / COLLISION_POINTS_PER_VECTOR) * i);
			} else {
				/* Theoretical position for non-visible fish:
				 * set Y to the average habitat depth. 
				 * set X to the Left or Right border, depending on the radar blip. */
				struct radar_blip *blip = NULL;

				for (int i = 0; i <= drone->blip_count; i++) {
					if (drone->blips[i].creature_id == id) {
						blip = drone->blips + i;
						break;
					}
				}
				...
				switch (fish->type) {
					case 0: fish_pos.y = 3750; break;
					case 1: fish_pos.y = 6250; break;
					case 2: fish_pos.y = 8750; break;
			...
			int distance = vec2d_distance(drone_pos, fish_pos);

			if (distance <= DRONE_FISH_SCAN_DISTANCE) {
				fish_scores[id] = MAX_FISH_VALUE;
				continue;
			}

			int fish_score = 3333 + compute_fish_value(fish);
			fish_score -= distance / 300;
			fish_scores[id] = fish_score;
		}
	}

	int vector_score = 0;
	for (int i = 0; i < ARRLEN(fish_scores); i++) {
		vector_score += fish_scores[i];
	}

	int drone_scans_value = 0;
	for (int i = 0; i < drone->scan_count; i++) {
		struct fish *fish = &state.entities[drone->scans[i]].fish;
		drone_scans_value += compute_fish_value(fish) * 20;
	}

	vector_score += drone_scans_value - ((drone->y + vector.y) / 1000);
